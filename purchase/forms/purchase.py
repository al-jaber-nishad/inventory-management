...
class PurchaseForm(forms.ModelForm):
    ...
    def generate_invoice_number(self):
        ...
        # Find the last purchase of this year
        ...
        if last_purchase:
            try:
                last_number = int(last_purchase.invoice_number.split('-')[-1])
    ...
    def clean_invoice_number(self):
        ...
        existing = Purchase.objects.filter(invoice_number=invoice_number)
        if self.instance.pk:
    ...
    def clean(self):
        ...
        if purchase_date and due_date and due_date < purchase_date:
            raise ValidationError("Due date cannot be earlier than purchase date.")
