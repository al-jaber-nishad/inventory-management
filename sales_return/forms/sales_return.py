...
class SaleReturnForm(forms.ModelForm):
    ...
    class Meta:
        model = SaleReturn
        fields = [
            'payment_ledger', 'customer', 'original_sale', 'return_number', 'return_date',
        ...
        widgets = {
            ...
            'original_sale': forms.Select(attrs={'class': 'form-control select2_search'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Filter original sales to show only confirmed/delivered sales
        self.fields['original_sale'].queryset = Sale.objects.filter(
            status__in=[Sale.Status.CONFIRMED, Sale.Status.DELIVERED]
        ).order_by('-sale_date')

    def generate_return_number(self):
        import datetime
        today = datetime.date.today()
        prefix = f"SR-{today.year}-"
        
        # Find the last sale return of this year
    ...
    def clean_return_number(self):
        ...
        existing = SaleReturn.objects.filter(return_number=return_number)
        if self.instance.pk:
    ...
    def clean(self):
        ...
        # If original sale is selected, customer should match
        if original_sale and customer and original_sale.customer != customer:
            raise ValidationError("Customer must match the original sale customer.")
        
        # If original sale is selected, auto-set customer
        if original_sale and not customer:
            cleaned_data['customer'] = original_sale.customer
...
class SaleReturnItemForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['product', 'original_sale_item', 'quantity', 'unit_price']
        widgets = {
            'product': forms.Select(attrs={'class': 'form-control product-select select2_search'}),
            'original_sale_item': forms.Select(attrs={'class': 'form-control original-item-select select2_search'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['original_sale_item'].queryset = SaleItem.objects.all()
        self.fields['original_sale_item'].required = False
    ...
    def clean_quantity(self):
        ...
        # If original sale item is selected, check if return quantity doesn't exceed sold quantity
        original_item = self.cleaned_data.get('original_sale_item')
        if original_item:
    ...
    def clean(self):
        ...
        original_item = cleaned_data.get('original_sale_item')
        
        # If original sale item is selected, product should match
        if original_item and product and original_item.product != product:
            raise ValidationError("Product must match the original sale item.")
        
        # If original sale item is selected, auto-set product and unit price
