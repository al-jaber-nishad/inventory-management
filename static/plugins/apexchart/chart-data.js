'use strict'; $(document).ready(function () {
    ...
    //             { name: 'Revenue', data: [76, 85, 101, 98, 87, 105, 91, 114, 94] }], 
    //             xaxis: { categories: ['Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct'], }, yaxis: { title: { text: '$ (thousands)' } }, fill: { opacity: 1 }, 
        ...
        $.ajax({
            ...
            success: function (data) {
                var sCol = {
                    ...
                    series: [
                        { name: 'Sent', data: data.received },
                        { name: 'Pending', data: data.pending },
                        { name: 'Failed', data: data.failed },
                        { name: 'API Failed', data: data.api_failed }
                    ],
                    xaxis: {
                    ...
                    tooltip: {
                        y: {
                            formatter: function (val) { return val + " messages" }
                        }
                    }
                }
                var chart = new ApexCharts(document.querySelector("#s-col"), sCol);
